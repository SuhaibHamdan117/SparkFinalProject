...
namespace Spark.DB.Repositories.AplicationUserRepository
{
    public class AplicationUserRepository : IAplicationUserRepository
    {
        ...
        public async Task<List<ApplicationUserDto>> GetAllUsersAsync(string userRole)
        {
            var c = await _userManager.GetUsersInRoleAsync(userRole);
            List<ApplicationUserDto> ApplicationUserDtoList = new List<ApplicationUserDto>();
            foreach (var user in c) {
                ApplicationUserDtoList.Add(_mapper.Map<ApplicationUserDto>(user));
            }
        }
    }
}
